...
video_output_dir = os.path.join(project_root, "videos", "9x9_hard_mazes")
...
def main():
    ...
    model_path_stage_b = os.path.join(project_root, "models", "stage_b", "best_model.zip")
    model_path_stage_a = os.path.join(project_root, "models", "stage_a_failed_9", "best_model.zip")
    
    if os.path.exists(model_path_stage_b):
        model_path = model_path_stage_b
    else:
        model_path = model_path_stage_a
    ...
    # Configure better video resolution (1920x1080)
    p.configureDebugVisualizer(p.COV_ENABLE_GUI, 0)  # Hide GUI overlays for cleaner video
    ...
    for ep in range(10):
        ...
        if success:
            ...
            timeouts += 1
            # Delete video for failed episodes to save space
